...
export async function POST(request: NextRequest) {
  try {
    ...
    if (codeRow.redeemed_at && codeRow.redeemed_email) {
    ...
    const redeemed = await redeemLeadAccessCode(codeRow.id, email);
    ...
    const expiresAt = new Date(Date.now() + campaign.session_duration_hours * 60 * 60 * 1000).toISOString();
